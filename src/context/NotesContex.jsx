...
const NotesProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await axios
          .get(`https://project-salus.onrender.com/data/dashboard/notes/${dashboardPage.userName}`)
          .then((response) => {
  ...
  const handleAddNewNote = () => {
    ...
    try {
      axios
        .post("https://project-salus.onrender.com/data/dashboard/notes", noteForSubmit)
        .then((response) => {
          if (response.status === 200) {
            setNoteMessage("");
            setNoteColor("");
  ...
  const handleDeleteNote = (noteId) => {
    try {
      axios
        .delete(
          `https://project-salus.onrender.com/data/dashboard/notes/${dashboardPage.userName}/${noteId}`
        )
        .then((response) => {
          if (response.status === 200) {
  ...
  const handleUpdateNote = () => {
    ...
    try {
      axios
        .put(
          `https://project-salus.onrender.com/data/dashoard/notes/update/${selectedNoteForEdit}`,
          noteForUpdate
        )
        .then((response) => {
          if (response.status === 200) {
